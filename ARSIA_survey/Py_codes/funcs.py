...
def generate_gaussian_clusters_v2(K, n, N, min_separation=3.0, random_seed=1):
    """
    Generate K Gaussian clusters where means are sufficiently far apart.
    Also generates the ground-truth means for each data point.

    Parameters:
    - K: Number of clusters.
    - n: Dimensionality of the data.
    - N: Number of points per cluster.
    - min_separation: Minimum distance between cluster means to ensure separation.
    - random_seed: Seed for reproducibility.

    Returns:
    - data: np.array of shape (N*K, n), containing generated points.
    - labels: np.array of shape (N*K,), containing cluster labels.
    - means: List of mean vectors for each cluster.
    - covariances: List of covariance matrices for each cluster.
    - GTmeans: np.array of shape (N*K, n), ground-truth means for each point.
    """
    np.random.seed(random_seed)
    data = []
    labels = []
    ...
    for k in range(K):
        # Ensure means are sufficiently far apart
        while True:
            mean = np.random.uniform(-5, 5, n)
            if all(np.linalg.norm(mean - np.array(m)) > min_separation for m in means):
                break
        
        means.append(mean)
        
        # Generate covariance with small variance to avoid overlap
        # covariance = np.eye(n) * np.random.uniform(0.5, 1)  # Scale the variance
        covariance = np.eye(n) 
        covariances.append(covariance)
        ...
        # Generate corresponding GT means (same shape as data)
        GTmeans.append((mean.reshape([n,1]) @ np.ones([1,N])).T)

        data.append(cluster_data)
        labels.extend([k] * N)

    # Stack the data and GTmeans together
    GTmeans = np.vstack(GTmeans)
    data = np.vstack(data)
    labels = np.array(labels)

    return data, labels, np.array(means), covariances, GTmeans

def generate_gaussian_clusters_customize(K, n, N, min_separation=3.0, random_seed=1):
    """
    Generate K Gaussian clusters where means are sufficiently far apart.
    Also generates the ground-truth means for each data point.
...
# Function to generate corner Gaussian clusters with one smaller variance cluster
def generate_gaussian_clusters_1small(K, n, N, overlap_factor=0.05, small_scale = 0.1, small_num_rate = 0.5, random_seed=1):
    """
    Generate Gaussian Clusters at the corners of unit boxes.
    One cluster has a smaller variance and fewer points.
    K >= 2.
    """
    np.random.seed(random_seed)
    random.seed(random_seed)
    ...
    # # Generate all possible corner means
    # means = np.array(list(product([1, -1], repeat=n)))
    for k in range(K):
        mean = np.random.uniform(-2, 2, n)
        means.append(mean)
    means = np.array(means)
        
    # Randomly select K corners
    selected = random.sample(range(means.shape[0]), K)
    
    # Choose one cluster to have smaller variance and fewer points
    special_cluster_idx = random.choice(selected)
    
    for k in selected:
        mean = means[k, :]  # Get mean coordinate
        
        
        if k == special_cluster_idx:
            mean = mean + 2
            selected_means.append(mean)  # make this mean away from others.
            covariance = small_scale * np.eye(n)  # Smaller variance
            num_points = int(N * small_num_rate)  # Fewer points
        else:
            selected_means.append(mean)
            covariance = overlap_factor * np.eye(n)  # Normal variance
            num_points = N  # Regular number of points
            
        covariances.append(covariance)
        cluster_data = np.random.multivariate_normal(mean, covariance, num_points)
        
        data.append(cluster_data)
        labels.extend([k] * num_points)
...
def generate_2half_circles(num_points=8, side_scale = 1, radius = 1, vertical_scale=0.5, inner_scale = 1, random_seed=1):
    """
    Generate a 2 half-circles dataset with two clusters.
    ...
    Returns:
    - dataX (np.ndarray): Data matrix (2, n), where each column is a data point.
    - labels (np.ndarray): Label array (n,), with labels 0 and 1 for each half-circle.
    - GTmeans (np.ndarray): Ground-truth mean matrix (2, n), where each column is the ground-truth mean for each data point.
    - means (np.ndarray): Ground-truth means of each cluster (2, 2), each column is a mean for one cluster.
    """
    np.random.seed(random_seed)

    # Generate first half-circle (label = 0)
    theta1 = np.linspace(0, np.pi, num_points)
    x1 = radius * np.cos(theta1)
    y1 = vertical_scale * radius * np.sin(theta1)
    label1 = np.zeros(num_points)

    # Generate second half-circle (label = 1)
    theta2 = np.linspace(0, np.pi, num_points)
    x2 = side_scale - radius * np.cos(theta2)
    y2 = vertical_scale * (-radius * np.sin(theta2)) + inner_scale
    label2 = np.ones(num_points)

    # Combine the two half-circles
    X1 = np.vstack((x1, y1)).T  # Shape (num_points, 2)
    X2 = np.vstack((x2, y2)).T  # Shape (num_points, 2)
    X = np.vstack((X1, X2))  # Shape (2 * num_points, 2)
    ...
    # Generate ground-truth means for each data point
    GTmeans = np.array([mean1] * num_points + [mean2] * num_points).T  # Shape (2, n)
...
# # Example usage
# dataX, labels, GTmeans, means = generate_2half_circles()

# # Convert to DataFrame for visualization
# df_circles = pd.DataFrame(dataX.T, columns=["Feature 1", "Feature 2"])
# df_circles["Label"] = labels

# # Plot the generated dataset
# plt.figure(figsize=(8, 6))
# plt.scatter(dataX[0, labels == 0], dataX[1, labels == 0], color='blue', label='Class 0 (Half-Circle 1)')
# plt.scatter(dataX[0, labels == 1], dataX[1, labels == 1], color='red', label='Class 1 (Half-Circle 2)')
# plt.scatter(means[0, :], means[1, :], color='black', marker='x', s=100, label='Ground-Truth Means')  # Mark means
# plt.xlabel("Feature 1")
# plt.ylabel("Feature 2")
# plt.legend()
# plt.title("Generated 2 Half-Circles Dataset with Ground-Truth Means")
# plt.show()

def generate_half_circle_ball(num_points_circle, num_points_ball, circle_center, ball_center, circle_radius = 1, ball_radius = 0.5, random_seed=1):
    """
    Generate a half-circle with a ball dataset with two clusters.

    Parameters:
    - num_points_circle (int): Number of core points in half-circle.
    - num_points_ball (int): Number of points in a ball
    - radius: as of their names.
    ...
    # Generate first half-circle (label = 0)
    theta1 = np.linspace(0, np.pi, num_points_circle)
    x1 = circle_center[0] + circle_radius * np.cos(theta1)
    y1 = circle_center[1] + circle_radius * np.sin(theta1)
    label1 = np.zeros(num_points_circle)

    # Generate second half-circle (label = 1)
    theta2 = np.linspace(0, 2 * np.pi, num_points_ball + 1)[:-1]
    x2 = ball_center[0] + ball_radius * np.cos(theta2)
    y2 = ball_center[1] + ball_radius * np.sin(theta2)
    label2 = np.ones(num_points_ball)
    ...
    # Generate ground-truth means for each data point
    GTmeans = np.array([mean1] * num_points_circle + [mean2] * num_points_ball).T  # Shape (2, n)
...
# # Generate Half-circle ball data
# num_points_circle = 10 
# num_points_ball = 10
# circle_center = np.array([0, 0])
# ball_center = np.array([0, -2])
# circle_radius = 4
# ball_radius = 1
# rand_seed = 3
# dataX, labels, ground_true_means, means = funcs.generate_half_circle_ball(num_points_circle, num_points_ball, circle_center, ball_center, circle_radius, ball_radius, random_seed=rand_seed)
...
def generate_hyperspheres(K, centers_mat, num_points_vec, radius=1.0, random_seed = 1):
    """
    Generate K circles (2D), spheres (3D), or hyperspheres (pD) with given centers, radius, 
    and number of points on each, ensuring uniform distribution on the hyperspheres.
    
    Parameters:
        K (int): Number of hyperspheres.
        centers_mat (ndarray): A (p x K) array where each column represents a center.
        num_points_vec (array): An array of length K, where each element represents the number of points.
        radius (float or array): Radius of the hyperspheres. Can be a single float for all or an array of length K.
        
    Returns:
        dataX (ndarray): Data points as an array of shape (p, total_points).
        y_true (ndarray): Labels indicating which hypersphere each point belongs to.
        true_cls_centers (ndarray): The input centers as a (p x K) array.
    """
    assert centers_mat.shape[1] == K, "centers_mat should have K columns (one per hypersphere)."
    assert len(num_points_vec) == K, "num_points_vec should have length K."
    
    p = centers_mat.shape[0]  # Dimension of space
    
    if isinstance(radius, (int, float)):
        radius = np.full(K, radius)  # If single radius, use it for all hyperspheres
    ...
    for k in range(K):
        num_points = num_points_vec[k]
        center = centers_mat[:, k]  # dim-p
        
        if p == 2:  # 2D case: generate a circle
            angles = np.linspace(0, 2 * np.pi, num_points, endpoint=False)
            sphere_points = np.vstack([
                center[0] + radius[k] * np.cos(angles),
                center[1] + radius[k] * np.sin(angles)
            ])
            
        else:  # Higher dimensions: generate evenly distributed points on a hypersphere
            # sampler = qmc.Sobol(d=p, scramble=True)
            # all_sobol_samples = sampler.random(2 ** p)  # Generate Sobol sequence for better uniformity
            # # select num_points from them.
            # selected_indices = np.random.choice(2 ** p, num_points, replace=False)
            # sobol_samples = all_sobol_samples[selected_indices,:]
            # # Convert Sobol sequence to points on a hypersphere
            # # random_directions = np.random.randn(p, num_points)  # Sample from normal distribution
            # # random_directions = sobol_samples.T
            # # random_directions /= np.linalg.norm(random_directions, axis=1)  # Normalize to unit sphere
            
           # Sample from a normal distribution and normalize
           random_directions = np.random.randn(p, num_points)
           random_directions /= np.linalg.norm(random_directions, axis=0) 
           
           sphere_points = center[:, None] + radius[k] * random_directions  # Scale and shift by center
        
        GTmeans.append((center.reshape([p,1]) @ np.ones([1,num_points])).T)
        dataX.append(sphere_points)
        y_true.extend([k] * num_points)  # Label each point with the corresponding hypersphere index
    
    dataX = np.hstack(dataX)  # Stack all points into a single array
    y_true = np.array(y_true)  # Convert labels to numpy array
    true_cls_centers = centers_mat.T  # Keep original centers
    GTmeans = np.vstack(GTmeans)
    
    return dataX, y_true, true_cls_centers, GTmeans
...
def dt_pre_log_scale(dataX, base=10):
    """
    Apply log transformation and standardization to reduce numeric differences in a data matrix.

    Parameters:
    dataX : numpy array (p × n) 
        The input data matrix with p features and n samples.
    base : float
        Logarithm base, default is 10. Can be np.e for natural log.

    Returns:
    dataX_log_scaled : numpy array (p × n)
        Log-transformed and standardized data matrix.
    """
    # Shift data to make all values positive
    shift = 1e-6  # Small constant to prevent log(0)
    dataX_shifted = dataX - np.min(dataX) + shift  
    
    # Apply log transformation
    if base == np.e:
        dataX_log = np.log(dataX_shifted)
    elif base == 10:
        dataX_log = np.log10(dataX_shifted)
    else:
        dataX_log = np.log(dataX_shifted) / np.log(base)  # Custom base
    
    # Standardization (zero mean, unit variance)
    # scaler = StandardScaler()
    # dataX_log_scaled = scaler.fit_transform(dataX_log)
    
    dataX_log_scaled = normalize(dataX_log, norm='l2', axis=1)
    
    return dataX_log_scaled


def select_high_variance_samples(dataX, k):
    """
    Selects k samples (columns) with the highest variance from the dataX matrix.

    Parameters:
    - dataX: numpy array of shape (p, n), where p is the number of features, and n is the number of samples.
    - k: number of samples to select.

    Returns:
    - selected_dataX: numpy array of shape (p, k) containing the selected samples.
    - selected_indices: list of indices of selected samples.
    """
    # Compute variance across features (row-wise variance across columns)
    sample_variances = np.var(dataX, axis=0)  # Variance across features for each sample
    
    # Get indices of top-k samples with the highest variance
    top_k_indices = np.argsort(sample_variances)[-k:]  # Get indices of k highest variances

    # Select the corresponding samples
    selected_dataX = dataX[:, top_k_indices]

    return selected_dataX, top_k_indices

# # Example usage:
# p, n = 100, 10000  # Example dimensions
# dataX = np.random.rand(p, n)  # Simulated dataset

# k = 500  # Select top 500 samples with highest variation
# selected_dataX, selected_indices = select_high_variance_samples(dataX, k)

# print("Selected sample indices:", selected_indices)
# print("Shape of selected data:", selected_dataX.shape)